...
const KdsFavorite = class {
  ...
  render() {
    const { favorited, what, disabled, buttonClass } = this;
    const theme = document.documentElement.getAttribute("kds-theme");
    const IconFavorited = theme === "admin" ? "kds-icon-ratings-solid" : "kds-icon-favorite-solid";
    const Icon = theme === "admin" ? "kds-icon-ratings" : "kds-icon-favorite";
    return (h(Host, null, h("button", { "aria-label": `${favorited ? "Remove" : "Save"} ${what} ${favorited ? "from" : "to"} my favorites.`, "aria-pressed": `${Boolean(favorited)}`, ref: el => (this.buttonRef = el), onClick: () => this.buttonRef.setAttribute("aria-pressed", `${!Boolean(favorited)}`), class: {
        "kds-Favorite": true,
        "kds-Favorite--favorited": favorited,
        [buttonClass]: Boolean(buttonClass)
      }, disabled: disabled }, h(IconFavorited, { hidden: !favorited, color: "interactive" }), h(Icon, { hidden: favorited }))));
  }
};
KdsFavorite.style = favoriteCss;

export { KdsFavorite as kds_favorite };
