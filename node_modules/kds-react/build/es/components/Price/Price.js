...
var Price = function Price(_ref) {
  ...
  return React.createElement("data", _extends({
    value: useAlternateStyle ? combinePrice(value) : value,
    typeof: "Price",
    className: priceClasses,
    "aria-label": priceAriaLabel
  }, props), React.createElement("meta", {
    name: "priceCurrency",
    content: "USD"
  }), hasRelativeCopy && React.createElement("span", _extends({
    className: relativePrefixClasses
  }, relativePrefixRestProps), relativePrefix, "\u200B"), original || useAlternateStyle ? React.createElement(React.Fragment, null, useAlternateStyle ? React.createElement("mark", _extends({
    className: cx(valueClasses, 'kds-Price-promotional--decorated')
  }, valueRestProps), React.createElement("sup", {
    className: "kds-Price-superscript"
  }, showOnlyCents ? hasNegativePrice(value) ? "".concat(minus) : '' : hasNegativePrice(value) ? "".concat(minus, "$") : '$'), React.createElement("span", {
    className: "kds-Price-promotional-dropCaps"
  }, showOnlyCents ? absoluteNum(value.cents) : formatWithCommas(absoluteNum(value.dollars))), React.createElement("sup", {
    className: "kds-Price-superscript"
  }, showOnlyCents ? '¢' : React.createElement(React.Fragment, null, React.createElement("span", {
    className: "screen-reader"
  }, "."), absoluteNum(value.cents) < 10 ? "0".concat(absoluteNum(value.cents)) : "".concat(absoluteNum(value.cents))))) : React.createElement("mark", _extends({
    className: valueClasses
  }, valueRestProps), curPrice), !!oriPrice && (dangerouslyHideOriginalPrice ? React.createElement("span", {
    className: "screen-reader"
  }, oriPrice) : React.createElement(React.Fragment, null, React.createElement("span", {
    className: "screen-reader"
  }, " discounted from "), React.createElement("s", _extends({
    className: originalValueClasses
  }, originalValueRestProps), oriPrice)))) : React.createElement("span", _extends({
    className: singularValueClasses
  }, valueRestProps), curPrice), hasRelativeCopy && React.createElement("span", _extends({
    className: relativePostfixClasses
  }, relativePostfixRestProps), "\u200B", relativePostfix, "\u200B"), quantity && React.createElement("small", _extends({
...
var Price$1 = React.memo(Price);

export default Price$1;
