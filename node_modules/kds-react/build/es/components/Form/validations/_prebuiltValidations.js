...
import { isValidNonEmptyString } from '../../../utils/stringUtils.js';
...
var minLength = function minLength(length) {
  ...
  return {
    _id: MIN_LENGTH,
    text: isValidNonEmptyString(text) ? text : "Input must be ".concat(length, " characters or more."),
    validate: function validate(val) {
...
var maxLength = function maxLength(length) {
  ...
  return {
    _id: MAX_LENGTH,
    text: isValidNonEmptyString(text) ? text : "Input must be ".concat(length, " characters or less."),
    validate: function validate(val) {
      return Boolean(val) && typeof val === 'string' && val.length <= length;
    }
  };
...
var min = function min(num) {
  ...
  return {
    _id: MIN,
    text: isValidNonEmptyString(text) ? text : "Input must be ".concat(num, " or more."),
    validate: function validate(val) {
...
var max = function max(num) {
  ...
  return {
    _id: MAX,
    text: isValidNonEmptyString(text) ? text : "Input must be ".concat(num, " or less."),
    validate: function validate(val) {
      return Number(val) <= num;
    }
  };
...
export { addRequiredToValidations, contains, max, maxLength, min, minLength, pattern, required };
