...
var getCharCount = function getCharCount(count) {
  var parsedCount = isNaN(parseInt(count)) ? count : parseInt(count).toString();
  return parsedCount.length;
};

var badgeColorOpts = ['brand', 'inverse', 'positive', 'negative', 'neutral', 'special', 'callout'];
/**
...
var Badge = function Badge(_ref) {
  var children = _ref.children,
      ...
      plus = _ref.plus,
      badgeProps = _objectWithoutProperties(_ref, ["children", "color", "count", "inverse", "plus"]);

  /**
   * The `count` prop can be either a string or a number, so a method to correctly handle either case is necessary.
   */
  var characterCount = plus || count === undefined ? 1 : getCharCount(count);
  var isCircular = characterCount === 1; // Keeping this for backwards compatibility for "inverse" prop

  if (inverse) {
    color = 'inverse';
  }

  return React__default.createElement("output", _extends({}, badgeProps, {
    className: cx('kds-Badge-wrapper', badgeProps.className)
  }), children, (count !== undefined || plus) && React__default.createElement("svg", {
    className: cx('kds-Badge', {
      'kds-Badge--pinned': Boolean(children)
    }, badgeColorOpts.includes(color) && "kds-Badge--".concat(color)) // TODO: Need to find a way to consume space-8 token from kds-design-tokens
    ,
    style: {
      width: "calc(".concat(characterCount, "ch + 0.50rem)")
    }
  }, isCircular ? React__default.createElement("rect", {
    width: "100%",
    height: "100%",
    ry: "100%"
    ...
    height: "100%",
    ry: "50%"
  }), plus ? React__default.createElement(Plus.IconPlus, {
    width: "100%",
    height: "100%",
    size: "xs"
...
exports.badgeColorOpts = badgeColorOpts;
