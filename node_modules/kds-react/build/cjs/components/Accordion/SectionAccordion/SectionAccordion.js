...
function (_PureComponent) {
  _inherits(SectionAccordion, _PureComponent);

  var _super = _createSuper(SectionAccordion);

  function SectionAccordion() {
    var _this;

    _classCallCheck(this, SectionAccordion);
    ...
    _defineProperty(_assertThisInitialized(_this), "uniqueId", _this.props.id || "kds-SectionAccordion-".concat(generationUtils.generateUniqueString()));

    _defineProperty(_assertThisInitialized(_this), "handleClick", function (event) {
      var open = !_this.state.open;
      var accordion = document.getElementById(_this.uniqueId);
      var accContent = accordion.querySelector('.kds-SectionAccordion-content');
      var expandedHeight = accContent.scrollHeight; // If collapsing, we need to set the height to px value for CSS transition to work BEFORE setting state.
  ...
  _createClass(SectionAccordion, [{
    ...
    value: function render() {
      var _this$props = this.props,
          ...
          Tag = _this$props.tag,
          _this$props$expandBut = _this$props.expandButtonText,
          _this$props$collapseB = _this$props.collapseButtonText,
          restProps = _objectWithoutProperties(_this$props, ["kind", "className", "defaultOpen", "title", "children", "onClick", "tag", "expandButtonText", "collapseButtonText"]);

      var open = this.state.open;

      var renderTitle = function renderTitle(title) {
        if (title && React__default.isValidElement(title) && title.type === Heading.default) {
          return React__default.cloneElement(title, {
      ...
      return React__default.createElement(Tag, _extends({
        id: this.uniqueId,
        className: cx('kds-SectionAccordion line-length', {
          'kds-SectionAccordion--open': open
        }, Tag !== 'li' ? 'border-t border-b border-default-400' : '', className)
      }, restProps), React__default.createElement("button", {
        id: "".concat(this.uniqueId, "-button"),
      ...
      }, renderTitle(title), React__default.createElement(CaretRight.IconCaretRight, {
        size: "s",
        className: "kds-SectionAccordion-icon kds-SectionAccordion-icon--".concat(open ? 'up' : 'down')
      })), React__default.createElement("div", {
        id: "".concat(this.uniqueId, "-content"),
        className: "kds-SectionAccordion-content",
        "aria-labelledby": "".concat(this.uniqueId, "-button"),
        tabIndex: "-1" // We dynamically calculate and add the px height for CSS transition to work, and then set it to 'auto' after animation is complete because we want this to be responsive to changes in orientation, screen width etc.
        ,
        ref: function ref(el) {
          if (el) {
            var expandedHeight = el.scrollHeight;
            el.setAttribute('style', "height: ".concat(open ? "".concat(expandedHeight, "px") : 0));
            setTimeout(function () {
              open && el.setAttribute('style', 'height: auto; visibility: visible');
            }, 500);
          }
        }
      }, React__default.createElement("div", {
        className: "kds-SectionAccordion-innerContent"
      }, children)));
    }
  }]);
...
_defineProperty(SectionAccordion, "defaultProps", {
  defaultOpen: false,
  tag: 'section'
});

exports.default = SectionAccordion;
