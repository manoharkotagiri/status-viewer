...
var DismissalButton = require('../DismissalButton/DismissalButton.js');
...
function (_PureComponent) {
  ...
  function Modal() {
    ...
    _defineProperty(_assertThisInitialized(_this), "uniqueId", _this.props.id || "kds-Modal-".concat(generationUtils.generateUniqueString()));
  ...
  _createClass(Modal, [{
    ...
    value: function getSnapshotBeforeUpdate(prevProps) {
      var modalIsOpening = this.props.isOpen && !prevProps.isOpen;

      if (modalIsOpening) {
        return windowUtils.getWindow().scrollY;
      }
    ...
    value: function componentDidUpdate(prevProps, prevState, snapshot) {
      var modalHasClosed = !this.props.isOpen && prevProps.isOpen;

      if (modalHasClosed && snapshot !== null) {
        windowUtils.getWindow().scrollTo(0, snapshot);
      }
    }
  }, {
    key: "configureModal",
    value: function configureModal(kind) {
      return {
        // MODAL CONFIGURATION
    ...
    value: function render() {
      ...
      var _this$props = this.props,
          ...
          PrimaryActionButton = _this$props.primaryActionButton,
          SecondaryActionButton = _this$props.secondaryActionButton,
          handleCloseModal = _this$props.handleCloseModal,
          children = _this$props.children,
          HeaderIcon = _this$props.headerIcon,
          onboardingStepCurrent = _this$props.onboardingStepCurrent,
          onboardingStepTotal = _this$props.onboardingStepTotal,
          onboardingHeaderImage = _this$props.onboardingHeaderImage,
          onAfterOpen = _this$props.onAfterOpen,
          _this$props$contentLa = _this$props.contentLabel,
          contentLabel = _this$props$contentLa === void 0 ? '' : _this$props$contentLa,
          _this$props$aria = _this$props.aria,
          aria = _this$props$aria === void 0 ? {} : _this$props$aria,
          props = _objectWithoutProperties(_this$props, ["headingText", "isOpen", "className", "kind", "primaryActionButton", "secondaryActionButton", "handleCloseModal", "children", "headerIcon", "onboardingStepCurrent", "onboardingStepTotal", "onboardingHeaderImage", "onAfterOpen", "contentLabel", "aria"]); // MODAL CONFIGURATION


      var _this$configureModal = this.configureModal(kind),
          isOnboarding = _this$configureModal.isOnboarding,
          allowClose = _this$configureModal.allowClose,
          showProgressIndicator = _this$configureModal.showProgressIndicator,
          showSecondaryButton = _this$configureModal.showSecondaryButton,
          showFooter = _this$configureModal.showFooter;

      var heading = React__default.createElement(React__default.Fragment, null, HeaderIcon && React__default.createElement(HeaderIcon, {
        size: "utility-l",
        className: "kds-Modal-headingIcon"
      }), React__default.createElement(Heading.default, {
      ...
      var computeAria = function computeAria() {
        ...
        var labelledby = aria.labelledby,
            ...
            rest = _objectWithoutProperties(aria, ["labelledby", "describedby"]);

        var computedDescribedby = _this2.props['aria-describedby'] || describedby || '';

        var computedAria = _objectSpread({
          modal: true
        }, rest);
      ...
      return React__default.createElement(ReactModal, _extends({
        isOpen: isOpen,
        ariaHideApp: false,
        onAfterOpen: functionUtils.callSafely(onAfterOpen),
        onRequestClose: functionUtils.callSafely(handleCloseModal),
        closeTimeoutMS: 100,
        overlayRef: function overlayRef(_overlayRef) {
          return _overlayRef && _overlayRef.setAttribute('role', 'presentation');
        },
        overlayClassName: {
        ...
        aria: computeAria(),
        id: "".concat(this.uniqueId),
        className: {
          base: cx("kds-Modal kds-Modal--l ".concat(isOnboarding ? 'kds-Modal--onboarding ' : '', "kds-Modal--").concat(kind), className),
      ...
      }, props), allowClose && React__default.createElement(DismissalButton.default, {
        className: "kds-Modal-closeButton",
        "aria-label": "Close pop-up",
        onClick: functionUtils.callSafely(handleCloseModal)
      }), React__default.createElement("header", {
        className: "kds-Modal-header"
      }, isOnboarding ? React__default.createElement("div", {
        className: "kds-Modal-headerImage"
      }, onboardingHeaderImage) : heading), React__default.createElement("div", {
        className: "kds-Modal-body"
      }, React__default.createElement("div", {
        className: "kds-Modal-content"
      }, isOnboarding && heading, children)), showProgressIndicator && React__default.createElement(Text.default, {
        size: "l",
        className: "kds-Modal-progressIndicator"
      }, "".concat(onboardingStepCurrent, " of ").concat(onboardingStepTotal)), showFooter && (PrimaryActionButton || SecondaryActionButton) && React__default.createElement("footer", {
        className: cx("kds-Modal-footer ".concat(showProgressIndicator ? 'kds-Modal-footer-columns' : ''), className)
      }, React__default.createElement("div", {
        className: "kds-Modal-footer-buttons"
      }, standardizeActionButton(PrimaryActionButton, 'primary'), showSecondaryButton && standardizeActionButton(SecondaryActionButton, "".concat(PrimaryActionButton ? 'secondary' : 'primary')))));
    }
  }]);
...
var isValidButtonKind = function isValidButtonKind(kind, validKinds) {
  return validKinds.filter(function (k) {
    return k === kind;
  }).length > 0;
};

var standardizeActionButton = function standardizeActionButton(button, actionKind) {
  if (button && React__default.isValidElement(button) && button.type === Button.default) {
    return React__default.cloneElement(button, {
      // Make sure button kind is valid. If not use default for the action type
      kind: isValidButtonKind(button.props.kind, validActionButtonKinds[actionKind]) ? button.props.kind : actionKind,
      className: cx(button.props.className, 'kds-Modal-actionButton', "kds-Modal-actionButton--".concat(actionKind))
    });
  }
