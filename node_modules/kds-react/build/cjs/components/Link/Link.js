...
var Download = require('../Icon/icons/Download.js');
...
var External = require('../Icon/icons/External.js');
...
var hasAbsoluteHref = function hasAbsoluteHref(href) {
...
var Link = function Link(_ref) {
  ...
  var href = _ref.href,
      ...
      noRouter = _ref.noRouter,
      withRouter = _ref.withRouter,
      size = _ref.size,
      children = _ref.children,
      aProps = _objectWithoutProperties(_ref, ["href", "implied", "explicit", "inverse", "noRouter", "withRouter", "size", "children"]);

  var validSize = sizeOpts.includes(size) ? size : 'inherit';
  /* Prevent invalid classes from being applied, default to 'inherit' */

  var classes = cx('kds-Link', (_cx = {}, _defineProperty(_cx, "kds-Link--".concat(validSize), size !== 'manual'), _defineProperty(_cx, 'kds-Link--implied', implied), _defineProperty(_cx, 'kds-Link--explicit', explicit), _defineProperty(_cx, 'kds-Link--inverse', inverse), _cx), aProps.className);

  if (typeof noRouter !== 'undefined') {
    deprecationWarning.deprecationWarning("'noRouter' prop in kds-react Link is deprecated and will soon be removed. No action is required http://design.kroger.com/#/react/components/link");
  }

  if (!noRouter && typeof withRouter === 'undefined') {
    deprecationWarning.deprecationWarning("kds-react Link default behavior is changing to render an 'a' tag by default. Use 'withRouter' prop to continue rendering a react-router NavLink http://design.kroger.com/#/react/components/link");
  }

  var isExternal = aProps.rel && aProps.rel.includes('external');
  var isDownload = aProps.download;
  var isMailTo = href && /^mailto:/.test(href);
  var isTel = href && /^tel:/.test(href);
  var temporaryWithRouter = withRouter || typeof withRouter === 'undefined'; // Temporarily allow withRouter to be truthy when undefined in order to detect what consumers are passing so we can properly alert them

  var temporaryNoRouter = !withRouter && noRouter;
  var useRouter = temporaryWithRouter && !isExternal && !isDownload && !isMailTo && !isTel && !temporaryNoRouter && !hasAbsoluteHref(href);
  var Tag = useRouter ? reactRouterDom.NavLink : 'a';
  var routerNavLinkProps = {
    isActive: function isActive() {
      return !!aProps['aria-current'];
    }
    /* required to set 'aria-current' for react-router-dom NavLink */

  };

  var newProps = _objectSpread({}, useRouter ? routerNavLinkProps : {}, {}, aProps, {}, _defineProperty({}, useRouter ? 'to' : 'href', href));

  var iconSizeClass = inlineIconSizeMap[size] || '';

  var renderChildren = function renderChildren() {
    return React__default.Children.map(children, function (child) {
      return iconUtils.isKDSIcon(child) ? React__default.cloneElement(child, {
        className: cx(child.props.className, iconSizeClass)
      }) : child;
    });
  };

  return React__default.createElement(Tag, _extends({}, newProps, {
    className: classes
  }), renderChildren(), isExternal && React__default.createElement(External.IconExternal, {
    className: iconSizeClass
  }), isDownload && React__default.createElement(Download.IconDownload, {
    className: iconSizeClass
  }));
};
