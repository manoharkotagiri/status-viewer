/* eslint-disable */

/***********************************************************************************************
  THIS IS AN AUTO-GENERATED FILE. DO NOT MAKE CHANGES HERE

  Generated on: Wed Dec 09 2020 21:14:13 GMT+0000 (Coordinated Universal Time)
************************************************************************************************/

'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

function _interopDefault (ex) { return (ex && (typeof ex === 'object') && 'default' in ex) ? ex['default'] : ex; }

var _extends = _interopDefault(require('@babel/runtime/helpers/extends'));
var _objectWithoutProperties = _interopDefault(require('@babel/runtime/helpers/objectWithoutProperties'));
var React = require('react');
var React__default = _interopDefault(React);
var cx = _interopDefault(require('classnames'));
var SectionAccordion = require('../Accordion/SectionAccordion/SectionAccordion.js');

/**
 * `<AccordionGroup>` renders a `<ul>` element with `<SectionAccordion>` or `<ParagraphAccordion>` children as `<li>` elements.
 * > **Note:** `<SectionAccordion>` and `<ParagraphAccordion>` render a `<section>` element by default; `<AccordionGroup>` clones each child and passes `tag="li"` as a prop to override this default. If you pass a `SectionAccordion` or a `<ParagraphAccordion>` child wrapped in a higher-order component, either it will need to accept this prop and pass it along, or you will need to explicitly specify the tag prop on the wrapped child.
 */

var AccordionGroup = function AccordionGroup(_ref) {
  var className = _ref.className,
      children = _ref.children,
      props = _objectWithoutProperties(_ref, ["className", "children"]);

  var renderChildren = function renderChildren() {
    return React__default.Children.map(children, function (child, index) {
      var childClassName = child.props.className;
      var childClasses = cx({
        'border-t border-default-400': child.type === SectionAccordion.default,
        'border-b': child.type === SectionAccordion.default && index === children.length - 1
      }, childClassName);
      return React__default.cloneElement(child, {
        tag: 'li',
        className: childClasses,
        key: child.key || "accordion-".concat(index)
      });
    });
  };

  return React__default.createElement("ul", _extends({
    className: cx('kds-AccordionGroup list-reset', className),
    "aria-label": "Accordion Control Button Group"
  }, props), renderChildren());
};

AccordionGroup.defaultProps = {
  className: ''
};

exports.default = AccordionGroup;
//# sourceMappingURL=AccordionGroup.js.map
